...
function matchesBriefPlatform(card, platforms) {
 ...
 */
export function computeCampaignScores(input) {
    ...
    const fitEntries = new Map(Object.entries(input.fitScores ?? {}).map(([id, score]) => [normalizeId(id), score]));
    ...
    const brandFit = cards.length === 0
        ? 40
        : clamp(fitValues.reduce((a, b) => a + b, 0) / cards.length);
    basis.push(`Brand fit: ${scoredCount}/${cards.length} creators carry persisted campaign fit scores (unscored count 60).`);
    ...
    basis.push(`Audience match: ${onPlatform}/${cards.length} creators on brief platforms; tier-mix adherence ${tierAdherence}/100.`);
    // Reach — log-scale follower coverage (10K→20, 1M→60, 100M→100).
    const reach = totalFollowers <= 0 ? 10 : clamp(20 * Math.log10(Math.max(totalFollowers, 10) / 1000));
    ...
    for (const card of cards) {
        if (card.engagementRate == null || !card.followers)
            continue;
    ...
    const benchmark = Object.entries(ER_BENCHMARKS).find(([key]) => benchmarkPlatform.includes(key))?.[1] ?? 3.5;
    const engagementForecast = avgEr == null ? 50 : clamp(60 * (avgEr / benchmark), 20, 100);
    basis.push(avgEr == null
    ...
    if (budgetAmount > 0 && projectedImpressions > 0) {
        ...
        basis.push(`Budget efficiency: ~${Math.round(projectedImpressions).toLocaleString()} projected impressions against ${budgetAmount.toLocaleString()} ${facts?.budget?.currency ?? ""} (${impressionsPerUnit.toFixed(1)} per unit).`.replace("  ", " "));
    }
    else {
    ...
    const categories = new Set(cards.flatMap((c) => c.categories ?? []).map((c) => c.toLowerCase()).filter(Boolean));
    const diversityScore = Math.min(categories.size / 5, 1) * 60;
    const targetTiers = [...new Set(tierMix.map((t) => t.tier))];
    const coveredTiers = targetTiers.length === 0
        ? 1
        : targetTiers.filter((tier) => cards.some((c) => matchesTierLabel(tier, creatorTierOf(c)))).length / targetTiers.length;
    const contentCoverage = clamp(diversityScore + coveredTiers * 40);
    basis.push(`Content coverage: ${categories.size} content categories; ${Math.round(coveredTiers * 100)}% of strategy tiers filled.`);
    ...
    if (concentration > 0.5 && cards.length > 1)
        risk -= 15;
    if (offPlatform > 0 && (facts?.platforms?.length ?? 0) > 0)
        risk -= 10;
    if (tierMix.length > 0 && coveredTiers < 1)
        risk -= 10;
    risk = clamp(risk, 5, 95);
    basis.push(`Risk: ${unenriched} creator${unenriched === 1 ? "" : "s"} pending intelligence refresh; top creator holds ${Math.round(concentration * 100)}% of combined reach.`);
    const overall = clamp((brandFit * 20 +
        audienceMatch * 15 +
        reach * 15 +
