import { markStaleCampaignOutputs, regenerateStaleCampaignOutputs, } from "@/features/campaign-outputs/output-registry";
 ...
 */
export async function commitCreatorSlateAndRegeneratePlan(supabase, campaignObject, options = {}) {
    ...
    const creatorsData = reoptimized.sections.creators.data;
    ...
    if (slateIds.length > 0) {
        ...
        if (influencerIds.length > 0) {
            try {
                ...
                const cards = result.creators.map((c) => mapBrowseCreatorToSearchResult(c, facts?.platforms));
