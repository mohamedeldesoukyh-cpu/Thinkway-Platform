import { canonicalTierLabel, matchesInfluencerTier, normalizeInfluencerTier, } from "@/lib/creators/influencer-tier";
...
function normalizePlatformKey(platform) {
    ...
    if (v.includes("insta") || v === "ig")
        return "instagram";
    if (v.includes("tiktok") || v === "tt")
        return "tiktok";
    if (v.includes("youtube") || v === "yt")
        return "youtube";
    if (v.includes("snap"))
        return "snapchat";
    return v;
}
export function creatorTierOf(creator) {
...
export function matchesTierLabel(mixTier, tierLabel) {
...
export function allocateTierCounts(mix, targetCount) {
...
export function composeCreatorSlate(creators, options) {
    ...
    if (requestedPlatforms.length > 0) {
        const filtered = creators.filter((c) => requestedPlatforms.includes(normalizePlatformKey(c.platform)));
    ...
    for (const m of mix) {
        ...
        for (const creator of withTier) {
            ...
            if (used.has(creator.id))
                continue;
            if (!accepted.includes((creator.tier ?? "").toLowerCase()))
                continue;
            slate.push(creator);
            used.add(creator.id);
            remaining -= 1;
...
export function isTrendCampaign(facts) {
...
export function buildCreatorContentIdea(creator, facts, fallbackIndex = 0) {
    ...
    for (const { pattern, idea } of TREND_CONCEPTS) {
        if (pattern.test(categories))
            return idea.replace(/the official sound/gi, "the campaign creative");
    }
