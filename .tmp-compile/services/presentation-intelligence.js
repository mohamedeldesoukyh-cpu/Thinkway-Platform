import { detectIndustryFromBrief, getIndustryProfile, resolveClientFromBrief, } from "./industry-intelligence";
import { parseDurationWeeks, resolveGoLiveWeek, } from "./timeline-duration";
...
function extractSection(text, header) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (header.test(lines[i])) {
            ...
            if (inline)
                return stripMarkdown(inline);
            for (let j = i + 1; j < lines.length; j++) {
                const next = lines[j].trim();
                if (/^#{1,3}\s/.test(next) || /^[A-Z][^:]+:/.test(next))
                    break;
                if (next)
                    return stripMarkdown(next.replace(/^[-*•]\s*/, ""));
            }
        }
    }
    return undefined;
...
export function deriveCreativeConcepts(strategyText, summaryText) {
...
export function deriveCreatorMix(strategyText, summaryText, tierStrategy) {
...
export function getIndustryCreatorMix(industry) {
...
export function deriveContentPlan(strategyText, summaryText) {
    const combined = [strategyText, summaryText].filter(Boolean).join("\n");
    const industry = detectIndustryFromBrief(combined);
...
export function deriveWhyAiInsights(strategyText, summaryText, budgetText) {
...
export function deriveExecutiveStrategyFields(strategyText, audienceText, summaryText) {
    const combined = [strategyText, audienceText, summaryText].filter(Boolean).join("\n");
    const industry = detectIndustryFromBrief(combined);
    const profile = getIndustryProfile(industry, combined);
    const challengeByIndustry = {
    ...
    return {
        ...
        campaignObjective: extractSection(combined, /objective/i) ??
            extractSection(combined, /campaign objective/i) ??
            profile.campaignType,
        targetAudience: extractSection(combined, /target audience/i) ??
        ...
        keyMessage: extractSection(combined, /key message/i) ??
        ...
        contentPillars: profile.platforms.map((p) => `${p}: ${profile.campaignType.split("&")[0].trim()} content`),
...
const INTERNAL_MILESTONE_PATTERN = /brief|vendor|discovery|shortlist|kickoff|outreach|compliance review|strategy approval|creator selection|planning|qa|pre-launch|stakeholder/i;
export function isClientFacingTimelinePhase(phase) {
    return !INTERNAL_MILESTONE_PATTERN.test(phase);
}
function assignClientPhaseWeeks(durationWeeks) {
...
function resolveClientPhaseLabel(week, durationWeeks, phases) {
    if (durationWeeks === 1)
        return "Publishing Window";
    if (week === phases.start)
        return "Campaign Start";
    if (week === phases.goLive)
        return "Publishing Window";
    if (week === phases.end)
        return "Reporting";
    if (week === phases.end - 1 && phases.end - 1 > phases.goLive)
        return "Campaign End";
    if (week < phases.goLive)
        return "Content Production";
    return "Optimization";
}
function clientPhaseTemplate(phase) {
    switch (phase) {
        case "Campaign Start":
            return {
                phase,
...
export function applyTimelineCompletionStatus(weeks, isComplete) {
    if (!isComplete)
        return weeks;
    return weeks.map((week) => ({ ...week, status: "complete" }));
}
function assignKeyPhaseWeeks(durationWeeks) {
    return assignClientPhaseWeeks(durationWeeks);
}
export function deriveTimelineWeeks(durationWeeks, _industry, options) {
...
export function mergeMilestonesIntoWeeks(weeks, _milestones, durationWeeks, _goLiveWeek, options) {
...
export function buildTimelineWeeksForCampaign(durationWeeks, industry, milestones = [], options) {
...
export function deriveDiscoveryPipeline(totalCandidates, isSearching) {
    const stages = [
...
export function deriveGroundedStrategy(strategyText, audienceText, summaryText) {
    const combined = [strategyText, audienceText, summaryText].filter(Boolean).join("\n");
    const industry = detectIndustryFromBrief(combined);
    const profile = getIndustryProfile(industry, combined);
...
export function deriveSuccessProbability(strategyText, summaryText) {
    const combined = [strategyText, summaryText].filter(Boolean).join("\n");
    const industry = detectIndustryFromBrief(combined);
    const profile = getIndustryProfile(industry, combined);
    const weeks = parseDurationWeeks(combined);
...
export function deriveOpportunities(strategyText, summaryText) {
    const combined = [strategyText, summaryText].filter(Boolean).join("\n");
    const industry = detectIndustryFromBrief(combined);
    const byIndustry = {
        luxury: [
...
export function deriveExecutiveSummary(strategyText, audienceText, summaryText) {
    const combined = [strategyText, audienceText, summaryText].filter(Boolean).join("\n");
    const industry = detectIndustryFromBrief(combined);
    const profile = getIndustryProfile(industry, combined);
    const client = resolveClientFromBrief(combined);
