import { detectCurrencyFromSources, parseBudgetTotalFromText, parseDurationFromText, stripMarkdown, } from "../components/sections/shared/format-utils";
...
export function detectIndustryFromBrief(...sources) {
...
export function getIndustryProfile(industry, contextText) {
    ...
    if (weeks >= 12)
        reach = reach.replace(/\d+M/g, (m) => `${parseInt(m) * 2}M`);
    else if (weeks <= 4)
        reach = reach.replace(/(\d+)M/g, (_, n) => `${Math.max(1, Math.round(parseInt(n) * 0.6))}M`);
    return { industry, ...base, estimatedReach: reach };
}
/** MENA-relevant companies recognized without an explicit label (client or brand). */
export const KNOWN_COMPANY_PATTERN = /\b(e&(?!\w)|(?:Coca-Cola|BabyJoy|Adidas|Emirates NBD|Visit Egypt|Rolex|Pepsi|Etisalat|Vodafone|Orange Egypt|STC|Zain|Careem|Talabat|Nestl[eé](?:\s+Egypt)?|L'Or[eé]al(?:\s+Paris)?)\b)/i;
export function resolveClientFromBrief(text) {
...
export function getIndustryKpis(industry, budgetText) {
    ...
    const formatReach = (base) => total && total > 500000
...
export function getIndustryRisks(industry) {
    switch (industry) {
        case "luxury":
            return [
                {
...
export { parseDurationWeeks } from "./timeline-duration";
...
export function getBudgetAllocationReason(industry, category) {
    ...
    const match = Object.entries(reasons).find(([key]) => normalizedCategory.toLowerCase().includes(key.toLowerCase().split(" ")[0]));
    return (match?.[1] ?? {
...
export function getGroundedKpis(industry, budgetText) {
...
export function getIndustryBenchmarks(industry, contextText) {
