...
import { previewRecommendationsFromDraft, previewSlateIntelligenceFromDraft, previewVendorDecisionsFromDraft, } from "./studio-draft-preview";
/** Creator ids appear both raw and "inf:"-prefixed across sections — compare normalized. */
export function normalizeCreatorId(id) {
    return id.trim().replace(/^inf:/, "");
}
function sameCreator(a, b) {
    return normalizeCreatorId(a) === normalizeCreatorId(b);
}
function targetCreatorIdOf(change) {
    switch (change.kind) {
        case "add_creator":
...
export function getStudioDraft(campaignObject) {
 ...
 */
export function stageDraftChange(draft, change) {
    ...
    const cancelledAdd = change.kind === "remove_creator" &&
        dropped > 0 &&
        draft.changes.some((existing) => existing.kind === "add_creator" &&
            sameCreator(existing.creator.creatorId, changeCreatorId));
    if (cancelledAdd) {
        remaining = remaining.filter((existing) => !(existing.kind === "remove_creator" &&
...
export function unstageDraftChange(draft, creatorId) {
    return {
...
export function draftChangeForCreator(draft, creatorId) {
    if (!creatorId)
        return undefined;
...
const SLATE_DEPENDENT_SECTIONS = [
...
const REFRESH_DEPENDENT_SECTIONS = [
    "creator-recommendations",
    "kpi-forecast",
    "success-probability",
];
const SLATE_CHANGE_KINDS = new Set([
    "remove_creator",
    "add_creator",
    "replace_creator",
    "replace_shortlist",
...
export function outdatedSectionsForDraft(draft) {
    const outdated = new Set();
    for (const change of draft.changes) {
        const affected = change.kind === "refresh_intelligence"
...
export function withStudioDraft(campaignObject, draft) {
    const creatorsData = (campaignObject.sections.creators.data ?? {});
    const nextData = { ...creatorsData, studioDraft: draft };
...
export function updateDraftCreatorEnrichment(draft, creatorId, enrichmentStatus) {
    return {
        changes: draft.changes.map((change) => change.kind === "add_creator" && sameCreator(change.creator.creatorId, creatorId)
            ? { ...change, creator: { ...change.creator, enrichmentStatus } }
            : change),
        updatedAt: new Date().toISOString(),
    };
}
function reasoningForAddedCreator(ref) {
    const tier = resolveCreatorTierLabel({ followers: ref.followers });
    const sourceLabel = ref.source === "external_url" ? "added from a profile link" : "picked from Discovery";
    return {
...
function applyRoleOverrides(recommendations, changes) {
 ...
 */
export function applyStudioDraftChanges(campaignObject) {
    const draft = getStudioDraft(campaignObject);
    if (draft.changes.length === 0) {
        return { campaignObject, removedCreatorIds: [], addedCreatorIds: [], unappliedChanges: [] };
    }
    const creatorsData = (campaignObject.sections.creators.data ?? {});
    const beforeIds = new Set((creatorsData.recommendations?.creatorIds ?? []).map(normalizeCreatorId));
    const nextRecommendations = previewRecommendationsFromDraft(creatorsData.recommendations, draft.changes);
    const addedCreatorIds = nextRecommendations.creatorIds.filter((id) => !beforeIds.has(normalizeCreatorId(id)));
    const removedCreatorIds = [...beforeIds].filter((id) => !nextRecommendations.creatorIds.some((cid) => sameCreator(cid, id)));
    const vendorDecisions = previewVendorDecisionsFromDraft(creatorsData.vendorDecisions, draft.changes);
    ...
    const reasoningById = new Map(existingReasoning.map((entry) => [normalizeCreatorId(entry.creatorId), entry]));
    ...
    let slateIntelligence = previewSlateIntelligenceFromDraft(creatorsData.slateIntelligence, draft.changes);
    ...
    const appliedShortlistIds = shortlistChange
        ? [
            ...(creatorsData.appliedShortlistIds ?? []).filter((id) => shortlistChange.kind !== "replace_shortlist" ||
                id !== shortlistChange.payload.shortlistId),
            shortlistChange.payload.shortlistId,
        ]
        : creatorsData.appliedShortlistIds;
    const shortlistVendorDecisions = shortlistChange?.kind === "replace_shortlist" ? {} : vendorDecisions;
    const nextData = {
        ...creatorsData,
        recommendations: {
            ...nextRecommendations,
        ...
        ...(shortlistCreatorChanges.length > 0 ? { phase: "shortlist" } : {}),
        ...(shortlistChange &&
    ...
    return {
        ...
        removedCreatorIds: removedCreatorIds.map((id) => {
            const original = creatorsData.recommendations?.creatorIds?.find((cid) => sameCreator(cid, id));
            return original ?? id;
        }),
        addedCreatorIds,
        unappliedChanges: [],
