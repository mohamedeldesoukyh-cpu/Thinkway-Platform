...
import { deserializeCampaignObject, serializeCampaignObject, } from "@/features/campaign-intelligence";
import { getConversationWithMessages, updateMessageMetadata, } from "@/features/ai-workspace/services/conversation-service";
 ...
 */
export async function persistCampaignObjectOnMessage(conversationId, messageId, userId, patch) {
    ...
    if (!message?.metadata?.campaignObject)
        return null;
    const current = deserializeCampaignObject(message.metadata.campaignObject);
