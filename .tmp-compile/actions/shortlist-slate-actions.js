...
function filterShortlistsForCampaign(rows, clientName, brandName) {
    const clientKey = clientName?.trim().toLowerCase();
    const brandKey = brandName?.trim().toLowerCase();
    const scoped = rows.filter((row) => {
        if (brandKey && row.brand_name?.trim().toLowerCase() === brandKey)
            return true;
        if (clientKey && row.client_name?.trim().toLowerCase() === clientKey)
            return true;
...
export async function listStudioShortlistsAction(input) {
    try {
        ...
        const next = await persistCampaignObjectOnMessage(input.conversationId, input.messageId, userId, (object) => object);
        if (!next)
        ...
        const filtered = filterShortlistsForCampaign(rows.filter((row) => row.creator_count > 0), facts?.clientName, facts?.brandName);
        return {
            ok: true,
...
export async function stageShortlistToSlateAction(input) {
    try {
        ...
        if (!detail)
            return { ok: false, message: "Shortlist not found." };
