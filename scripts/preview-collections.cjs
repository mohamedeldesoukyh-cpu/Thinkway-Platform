...
(async () => {
  await esbuild.build({ stdin: { contents: entry, resolveDir: root, loader: 'tsx' }, bundle: true, outfile: path.join(out, 'app.js'), jsx: 'automatic', define: { 'process.env.NODE_ENV': '"development"' }, plugins: [{ name: 'preview-stubs', setup(build) {
    build.onResolve({ filter: /.*/ }, args => mocks[args.path] ? { path: args.path, namespace: 'mock' } : args.path.startsWith('@/') ? { path: path.join(root, args.path.slice(2) + (args.path.endsWith('currency-totals') || args.path.endsWith('redesign-data') ? '.ts' : '')) } : undefined);
