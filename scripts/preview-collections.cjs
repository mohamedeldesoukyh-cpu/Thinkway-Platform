...
const mocks = {
  ...
  '@/features/collections/redesign-actions': `export async function saveCollectionFollowUp(){return {ok:false,error:'Preview only: no history was saved.'}} export async function restoreCollectionPayment(){document.body.dataset.previewRestores=String(Number(document.body.dataset.previewRestores||0)+1);return {ok:false,error:'Preview only'}} export async function deleteCollectionPayment(){document.body.dataset.previewDeletes=String(Number(document.body.dataset.previewDeletes||0)+1);return {ok:false,error:'Preview only'}} export async function reviseCollectionPayment(){document.body.dataset.previewEdits=String(Number(document.body.dataset.previewEdits||0)+1);return {ok:false,error:'Preview only: no payment changed.'}}`,
};
(async () => {
