...
const outDir = path.join(repoRoot, "features/quotations/components");
const targets = [
...
for (const rel of targets) {
  ...
  if (contents && !corrupt) {
    fs.writeFileSync(path.join(repoRoot, rel), contents);
  } else if (contents) {
    fs.writeFileSync(path.join(repoRoot, "scripts/recovered", path.basename(rel)), contents);
  }
}
