...
BEGIN
 ...
 INSERT INTO creator_supplier_invoices(request_id,assignment_id,status,invoice_number,invoice_date,country_code,currency,supplier_name,subtotal,vat_amount,confirmed,recorded_by,notes) VALUES(gen_random_uuid(),a.id,'received',n,CURRENT_DATE-50,'EG',a.currency,'Existing supplier',a.cost_before_vat,0,true,actor,'Preserve original legal details');
 ...
 IF (SELECT count(*) FROM creator_supplier_invoices WHERE assignment_id=a.id)<>1 THEN RAISE EXCEPTION 'Duplicate invoice created';END IF;
