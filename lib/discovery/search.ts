...
const PROFILE_METRICS_FIELDS =
  "followers, following, posts_count, avg_likes, avg_comments, engagement_rate, avg_views, posting_frequency_per_week, reels_views_avg, captured_at";

const PROFILE_AI_SCORE_FIELDS =
  "category, niche, audience_type, content_quality_score, luxury_level_score, brand_fit_score, professionalism_score, influencer_summary, audience_persona, content_style, scored_at";
...
/** Limit nested metrics/AI rows to the latest snapshot per profile. */
function withLatestNestedRowLimits<
  Q extends {
    order: (
      column: string,
...
>(query: Q): Q {
  return query
    .order("captured_at", { referencedTable: "profile_metrics", ascending: false })
    .limit(1, { referencedTable: "profile_metrics" })
    .order("scored_at", { referencedTable: "profile_ai_scores", ascending: false })
    .limit(1, { referencedTable: "profile_ai_scores" });
}
...
export async function searchDiscoveredProfiles(
  ...
  if (searchQuery && rankByProfileId) {
    profiles.sort(
      (a, b) => (rankByProfileId.get(b.id) ?? 0) - (rankByProfileId.get(a.id) ?? 0)
    );
