...
export type DebitNotePayloadInput = {
...
export function buildDebitNotePayload(input: DebitNotePayloadInput): ErpDocumentHeader {
  const kind =
    input.party.type === "client" ? "client_debit_note" : "vendor_debit_note";
  ...
  return {
    ...
    metadata: {
      source_document_number: input.source_document_number ?? null,
