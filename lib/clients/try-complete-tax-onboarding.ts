...
export type TryCompleteTaxOnboardingResult = {
  completed: boolean;
  onboardingStatus?: ClientOnboardingStatus;
};

export async function tryCompleteTaxOnboarding(params: {
  supabase: Supabase;
  clientId: string;
  userId: string;
}): Promise<TryCompleteTaxOnboardingResult> {
  ...
  const readiness = assessClientTaxReadiness({
