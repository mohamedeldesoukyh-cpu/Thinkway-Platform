...
function testSettledParentPreservesPostLeavesWithRemaining() {
  const assignment: OperationalBillingRow = {
    id: "line-1",
    kind: "assignment",
    campaign_header_id: "camp-1",
    ...
    revenue_vat_percent: 0,
    revenue_vat_exempt: false,
    operational_status: "partially_invoiced",
    vendor_io_id: "vio-1",
    vendor_io_document_number: "VIO-1",
    pricing_mode: "per_deliverable",
    children: [
  ...
  const post2 = linked[0]?.children[0]?.children[1];
  assert(post2?.remaining_amount === 1000, "uninvoiced post leaf keeps remaining when parent settled");
  assert(post2?.is_locked === false, "uninvoiced post leaf stays unlocked when parent settled");
}
...
const tests = [
  ...
  testSettledParentPreservesPostLeavesWithRemaining,
];
