...
export function renderCampaignPerformanceReportHtml(
  ...
  const kpiRows = [
    ...
    ["CPM", formatMoneyValue(summary.average_cpm, summary.currency)],
    ["CPV", formatMoneyValue(summary.average_cpv, summary.currency)],
...
export async function buildCampaignPerformanceExcelBuffer(
  ...
  const headers = excelRows[0] ? Object.keys(excelRows[0]) : [
    "Creator",
    "Platform",
    "Type",
    "URL",
  ...
  const sheet: StyledSheetConfig = {
    ...
    columnFormats: headers.map((h) =>
      h === "ER %" ? "percent" : ["Cost", "CPV", "CPE"].includes(h) ? "money" : "text"
    ),
  };
