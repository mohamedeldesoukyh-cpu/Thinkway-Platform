-- Category Creator Search browse was timing out on Production (~8s statement_timeout)
-- when multi-filters (country + category + metrics) used browse_influencer_ids_for_categories.
-- count(*) OVER () forced a full filtered materialization before LIMIT, so even page-1
-- ID windows paid a full Beauty×country scan. Same class of fix as the recency fast path.
--
-- Keep the return signature (id, total_count) for PostgREST compatibility; total_count is
-- no longer computed (NULL). Callers use LIMIT+1 for has_more.
...
AS $$
  ...
  SELECT ranked.id, NULL::bigint AS total_count
  FROM ranked;
$$;
