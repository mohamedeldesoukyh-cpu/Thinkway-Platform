...
CREATE OR REPLACE FUNCTION public.save_creator_payment_invoice(p_assignment uuid,p_number text,p_date date,p_country text,p_vat boolean,p_revision integer,p_expected_vat numeric,p_override_reason text)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE a public.campaign_influencers; l public.campaign_lines; requires_override boolean; prior public.creator_supplier_invoices; invoice_correction boolean; invoice_count integer;
...
BEGIN
 ...
 IF prior.id IS NULL THEN
   SELECT count(*) INTO invoice_count FROM creator_supplier_invoices WHERE assignment_id=a.id;
   IF invoice_count>1 THEN RAISE EXCEPTION 'Multiple creator invoices exist. Review the invoice register before changing assignment VAT'; END IF;
   SELECT * INTO prior FROM creator_supplier_invoices WHERE assignment_id=a.id FOR UPDATE;
 END IF;
 IF prior.id IS NOT NULL AND prior.revision IS DISTINCT FROM p_revision THEN RAISE EXCEPTION 'Invoice changed. Refresh before saving'; END IF;
 invoice_correction:=prior.id IS NOT NULL AND (prior.vat_amount IS DISTINCT FROM round(l.cost_before_vat*rate/100,2) OR prior.subtotal IS DISTINCT FROM l.cost_before_vat);
 IF invoice_correction AND (reason IS NULL OR length(reason)<5 OR length(reason)>1000) THEN RAISE EXCEPTION 'Enter a Finance correction reason (5–1000 characters)'; END IF;
 -- Reuse the original row, keeping its attachment, legal details and audit trail.
 IF prior.id IS NOT NULL AND NOT prior.from_payment_register THEN
   UPDATE creator_supplier_invoices SET from_payment_register=true WHERE id=prior.id;
 END IF;
 ...
 END IF;
 IF invoice_correction THEN
   INSERT INTO finance_override_logs(entity_type,entity_id,override_type,reason,granted_by,granted_until,metadata)
   VALUES('creator_supplier_invoice',prior.id,'creator_invoice_vat_correction',reason,auth.uid(),clock_timestamp(),
     jsonb_build_object('assignment_id',a.id,'old_subtotal',prior.subtotal,'new_subtotal',l.cost_before_vat,'old_vat_amount',prior.vat_amount,'new_vat_amount',round(l.cost_before_vat*rate/100,2)));
 END IF;
END $$;
