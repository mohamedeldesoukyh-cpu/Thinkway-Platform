-- Preserve candidate admission; check the existing inference fallback before correlated lookups.
BEGIN;
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = pg_catalog, public, pg_temp AS $$
  WITH f AS (
    ...
    WHERE i.status='active'
      ...
      AND ((coalesce(i.country_code,'') !~ '^[A-Z]{2}$' AND cardinality(coalesce(i.country_codes,'{}'))=0) OR cardinality(f.countries)=0 OR lower(btrim(i.country_code))=ANY(f.country_values) OR i.country_codes && f.countries
