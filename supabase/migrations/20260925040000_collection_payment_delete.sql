...
BEGIN
 ...
 IF p.id IS NULL OR inv.id IS NULL OR p.status<>'completed' OR p_revision IS NULL OR p.revision<>p_revision THEN RAISE EXCEPTION 'Payment changed or unavailable. Reload before editing'; END IF;
