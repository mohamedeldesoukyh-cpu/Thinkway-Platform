export type VatEntry={id:string;date:string;name:string;invoice:string;country:string;currency:string;vatIn:number;vatOut:number;source:string};
...
export function vatMonths(data:VatLedger){
 ...
 for(const e of data.entries){const r=get(e.date.slice(0,7),e.country,e.currency);r.vatIn+=e.vatIn;r.vatOut+=e.vatOut;}
