import {vatInvoiceEligibility} from './ledger-model';
...
export async function loadVatLedger():Promise<VatLedger>{
 ...
 const [invoices,suppliers,payments,costs]=await Promise.all([all('invoices','id,document_number,issue_date,currency,status,regeneration_status,revenue_vat_amount,billing_country_code,client:clients(name,country)'),all('creator_supplier_invoices','*'),all('vat_authority_payments','*'),all('campaign_lines','id,currency_code,cost_vat_amount')]);
 ...
 return {entries:[...invoices.map(i=>({...vatInvoiceEligibility(i),id:i.id,date:i.issue_date??'',name:i.client?.name??'Not set',invoice:i.document_number,country:country(i.billing_country_code||i.client?.country),currency:i.currency,vatIn:Number(i.revenue_vat_amount??0),vatOut:0,source:'Client invoice'})),...suppliers.map(i=>({included:i.status==='received'&&i.confirmed,status:i.status==='received'?(i.confirmed?'confirmed':'Awaiting review'):i.status.replaceAll('_',' '),id:i.id,date:i.invoice_date??'',name:i.supplier_name,invoice:i.invoice_number,country:i.country_code,currency:i.currency,vatIn:0,vatOut:Number(i.vat_amount),source:'Creator invoice'}))],payments:payments.map(p=>({...p,amount:Number(p.amount)})),provisional:[...provisional].map(([currency,amount])=>({currency,amount}))};
}
