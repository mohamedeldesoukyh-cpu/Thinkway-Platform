    ...
    >
      {enabled.map((item) => (
        <button
          ...
          className={cn(
            "group inline-flex max-w-full items-center gap-1 rounded-full border border-[#9edfc8] dark:border-emerald-500/35 bg-[#ecfdf5] dark:bg-emerald-500/10",
            "py-1 pr-1.5 pl-2.5 text-[11px] font-medium text-[#168a66] dark:text-emerald-300 transition-colors hover:bg-[#d1fae5] dark:hover:bg-emerald-500/20"
          )}
        >
          <span className="truncate">{formatChipLabel(item)}</span>
          {item.weight > 0 ? (
            <span className="shrink-0 rounded bg-[#1D9E75]/15 dark:bg-emerald-500/20 px-1 py-px text-[10px] font-semibold text-[#168a66] dark:text-emerald-300">
              {item.weight}%
            </span>
          ) : null}
