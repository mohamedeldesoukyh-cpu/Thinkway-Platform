 ...
 * Mirrors `features/quotations/templates/quotation-template-html.ts`.
 */
...
function renderCoverPage(payload: ShortlistTemplatePayload): string {
  ...
  <div class="foot" style="border-top-color:rgba(205,216,245,.18); color:#7f8bb0;">
    <span>${esc(payload.footer.left)}</span><span>Generated ${esc(s.generatedDate)}</span>
  </div>
</section>`;
}

function renderCategoryTierPage(payload: ShortlistTemplatePayload): string {
  ...
  <div class="pad">
    ...
    ${tiers}
    <div class="grand tier-breakdown-grand-total">
    ...
    <div class="insight"><p><b>Roster insight.</b> ${esc(payload.insight.narrative)}${
      insightParts ? ` ${esc(insightParts)}` : ""
    }</p></div>
  </div>
  <div class="foot"><span>${esc(payload.footer.left)}</span><span class="mono">${esc(payload.shortlist.number)} · Page 2</span></div>
</section>`;
}
...
function renderShowcaseCreatorPages(
  ...
  <div class="foot"><span>${esc(payload.footer.left)}</span><span class="mono">${esc(payload.shortlist.number)} · ${esc(creator.handle)}</span></div>
</section>`;
    })
    .join("");
}
...
function renderRosterPage(
  ...
  <div class="pad">
    <div class="sec-row"><span class="sec-badge">${esc(payload.roster.sectionNo)}</span><span class="lbl">${esc(rosterTitle)} (${esc(payload.totals.creatorCount)})</span></div>
    <h2 class="sec-title">${esc(rosterTitle)}</h2>
    <p class="roster-note">${esc(payload.roster.note)}</p>
    <div class="fees">
      <table>
        <thead>${tableHead}</thead>
        <tbody>${rows || `<tr><td colspan="6" class="pub-empty">No creators on this shortlist.</td></tr>`}</tbody>
      </table>
    </div>
    <div class="company" style="margin-top:28px;">
      ${renderLogo("footer")}
      <div class="addr">${esc(payload.company.legalLine)}<br>${esc(payload.company.address)}</div>
    </div>
  </div>
  <div class="foot"><span>${esc(payload.footer.left)}</span><span class="mono">${esc(payload.shortlist.number)} · Roster</span></div>
</section>`;
}
...
export function buildShortlistTemplateHtml(
  ...
  const bodyClass = [
    payload.flags.showcaseCreators
      ? "shortlist-export-preview shortlist-showcase shortlist-report"
      : "shortlist-export-preview shortlist-report",
  ...
  const sections = [
    ...
    ...(payload.flags.showcaseCreators
      ? [renderShowcaseCreatorPages(payload, doc, siteOrigin)]
      : []),
