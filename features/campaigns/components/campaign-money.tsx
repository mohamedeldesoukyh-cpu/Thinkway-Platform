...
export function CampaignMoney({ amount, currency }: { amount: number; currency: string }) {
  const workspace = useCampaignCurrency();
  const target = workspace?.currency_code ?? currency;
  let converted: number;
  try { converted = convertMoney(amount, currency, target, workspace?.currency_rates ?? {}); }
...
export function campaignMoney(amount: number, currency: string) { return <CampaignMoney amount={amount} currency={currency} />; }
