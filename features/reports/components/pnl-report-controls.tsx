...
import type { PnlComparisonReportData } from "@/lib/analytics/pnl/pnl-report-types";
...
export function PnlReportControls({ report }: Props) {
  ...
  const state = useMemo(
    () => ({
      year: String(report.current_year),
      compare: String(report.prior_year),
    ...
    >
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="grid min-w-[7rem] gap-1">
          ...
          <Select value={state.compare} onValueChange={(value) => apply({ compare: value })}>
            <SelectTrigger className="h-8 w-[7rem] text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="grid min-w-[8rem] gap-1">
          ...
          <Select
            value={state.period}
            onValueChange={(value) =>
              apply({ period: value as typeof state.period })
            }
          >
        ...
        <div className="ml-auto">
          <ReportExportActions apiPath="/api/reports/pnl/document" />
        </div>
      </div>
    </div>
  );
