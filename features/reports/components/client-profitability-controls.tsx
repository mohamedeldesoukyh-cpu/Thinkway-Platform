...
export function ClientProfitabilityControls({ report, onClearSelection }: Props) {
  ...
  const state = useMemo(
    () => ({
      ...
      clients: report.selected_client_ids.join(","),
  ...
  const apply = useCallback(
    (patch: Partial<typeof state>) => {
      ...
      if (next.clients) {
        params.set("clients", next.clients);
      }
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        {hasSelection ? (
          <Button
            ...
            onClick={onClearSelection ?? reset}
          >
            View all clients
          </Button>
        ) : null}
        ...
        <div className="ml-auto">
          <ReportExportActions apiPath="/api/reports/client-profitability/document" />
        </div>
      </div>
    </div>
  );
