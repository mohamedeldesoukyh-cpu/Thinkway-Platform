...
import { TOP_INFLUENCERS_LIMIT_OPTIONS } from "@/lib/analytics/top-influencers/top-influencers-types";
...
export function TopInfluencersControls({ report }: Props) {
    ...
    >
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="grid min-w-[7rem] gap-1">
          ...
          <Select
            value={state.metric}
            onValueChange={(value) => apply({ metric: value as TopInfluencersMetric })}
          >
            <SelectTrigger className="h-8 w-[7rem] text-xs">
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="grid min-w-[6rem] gap-1">
          ...
          <Select value={state.limit} onValueChange={(value) => apply({ limit: value })}>
            <SelectTrigger className="h-8 w-[5.5rem] text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {TOP_INFLUENCERS_LIMIT_OPTIONS.map((count) => (
        ...
        <div className="grid min-w-[7rem] gap-1">
          ...
          <Select
            ...
            onValueChange={(value) =>
              apply({ clientType: value as TopInfluencersClientTypeFilter })
            }
          >
            <SelectTrigger className="h-8 w-[7rem] text-xs">
              <SelectValue />
        ...
        <div className="ml-auto">
          <ReportExportActions apiPath="/api/reports/top-influencers/document" />
        </div>
      </div>
    </div>
  );
