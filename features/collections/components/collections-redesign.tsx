...
function ReceiptForm({ invoices, preferred, asOf }: { invoices: CollectionInvoiceRow[]; preferred: string; asOf: string }) {
  ...
  return <form data-payment-create aria-busy={pending} className="tw-c" onSubmit={e => { e.preventDefault(); submit(); }}><Head title="Record client payment" subtitle="Record money received · partial receipts update the invoice balance" />
    ...
    <div className="tw-form">
      <div className="tw-f"><label className="tw-lbl" htmlFor="cr-invoice">Invoice</label><select className="tw-in" id="cr-invoice" required value={id} onChange={e => setId(e.target.value)}><option value="">Select invoice</option>{invoices.map(r => <option key={r.id} value={r.id}>{documentLabel(r.document_number)} · {r.client_name} · {r.currency} {money(r.outstanding)} open</option>)}</select></div>
