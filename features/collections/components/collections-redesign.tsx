        ...
        {<div hidden={!isOpen} className="tw-ad" style={{ borderLeftWidth: 0, boxShadow: "inset 3px 0 0 var(--tw-blue)" }}>
          {rows.length ? <>
            ...
            {rows.map(row => <Grid key={row.id} type="aging" variant="tw-adr" cells={[
              <Empty />, <span className="tw-t"><Link className="tw-code" href={`/billing/invoices/${row.id}`}>{documentLabel(row.document_number)}</Link><br />{dateLabel(row.issue_date)} → due {dateLabel(row.due_date)}</span>,
...
function ReceiptForm({ invoices: allInvoices, preferred, asOf }: { invoices: CollectionInvoiceRow[]; preferred: string; asOf: string }) {
  const router = useRouter();
  const [pending, start] = useTransition();
  const saving = useRef(false);
  const [invoiceScope, setInvoiceScope] = useState("unsettled");
  const invoices = invoiceScope === "all" ? allInvoices : allInvoices.filter(r => r.outstanding > 0);
  ...
  function submit() {
    if (saving.current || pending || !row || !date || date > asOf.slice(0, 10) || !Number.isFinite(value) || value <= 0 || value > row.outstanding) return;
    saving.current = true;
    ...
    start(async () => {
      ...
      } catch { toast.error("Could not save payment. Please retry."); } finally { saving.current = false; }
    });
  }
  return <form data-payment-create aria-busy={pending} className="tw-c" onSubmit={e => { e.preventDefault(); submit(); }}><Head title="Record client payment" subtitle="Record money received · partial receipts update the invoice balance"><label className="tw-f" style={{margin:0}}>Invoices<select className="tw-in" aria-label="Show invoices" value={invoiceScope} onChange={e => setInvoiceScope(e.target.value)}><option value="unsettled">Unsettled</option><option value="all">Show all invoices</option></select></label></Head>
    {!invoices.length && <div className="tw-note">No invoice matches this selection. Choose Show all invoices to include settled invoices, or change the client filter.</div>}
    <div className="tw-form">
      <div className="tw-f"><label className="tw-lbl" htmlFor="cr-invoice">Invoice</label><select className="tw-in" id="cr-invoice" required value={id} onChange={e => setId(e.target.value)}><option value="">Select invoice</option>{invoices.map(r => <option key={r.id} value={r.id}>{documentLabel(r.document_number)} · {r.client_name} · {r.currency} {money(r.outstanding)} {r.outstanding > 0 ? "open" : "· Settled"}</option>)}</select></div>
      <div className="tw-f"><label className="tw-lbl" htmlFor="cr-amount">Amount received</label><input className="tw-cwin" id="cr-amount" required max={row?.outstanding} type="number" min="0.01" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" /></div>
      ...
      <div className="tw-f"><label className="tw-lbl" htmlFor="cr-date">Value date</label><input className="tw-in" id="cr-date" required type="date" value={date} max={asOf.slice(0, 10)} onChange={e => setDate(e.target.value)} /></div>
    ...
    {row && row.outstanding <= 0 && <div className="tw-note">This invoice is settled. View or correct its receipts in Payment history below; no additional payment can be saved.</div>}
    ...
    <div className="tw-ch"><span className="tw-hint">Ctrl+S saves the payment. Save draft keeps details in this browser session; Load saved draft fills them back in without recording a payment.</span><span className="tw-sp" /><button type="button" className="tw-b" onClick={restore} title="Loads details saved in this browser session; does not restore a deleted payment.">Load saved draft</button><button type="button" className="tw-b" onClick={() => { sessionStorage.setItem("collections-receipt-draft", JSON.stringify({ id, amount, method, date, reference, notes })); toast.success("Draft saved in this browser session."); }}>Save draft</button><button className="tw-b pri" disabled={pending || !row || !date || !Number.isFinite(value) || value <= 0 || value > row.outstanding} type="submit" aria-keyshortcuts="Control+s Meta+s" title="Save payment (Ctrl+S)">{pending ? "Saving…" : "Save payment"}</button></div>
  </form>;
}
  ...
  return <div className="collections-suite" style={{ minWidth: 0, maxWidth: "100%", fontFamily: "var(--font-sans, Geist), sans-serif" }}>
    <div className="tw-mast">
      ...
      <div className="tw-mr" role="tablist" aria-label="Collections sections">{COLLECTION_TABS.map(([key, label]) => <button key={key} id={`tab-${key}`} role="tab" aria-controls={`panel-${key}`} aria-selected={tab === key} className={`tw-b sm${tab === key ? " pri" : ""}`} data-s={key} onClick={() => navigate(key)}>{label}{(key === "aging" || key === "overdue") && <span className="tw-cc">{key === "aging" ? open.length : overdue.length}</span>}</button>)}<span className="tw-sp" />
        <SearchableSelect id="collections-client" className="tw-in w-[190px]" value={client} searchPlaceholder="Search client name or code…" options={[{value: "", label: "All clients"}, ...data.clients.map(c => ({value: c.id, label: c.name, description: documentLabel(c.document_number) || undefined, keywords: [c.document_number ?? ""]}))]} onValueChange={value => { setStatementClient(value); navigate(tab, value); }} />
    ...
    <section data-p="dash" id="panel-dash" role="tabpanel" aria-labelledby="tab-dash" hidden={tab !== "dash"}><div className="tw-note">Choose a client or code and currency to review receivables. Open A/R aging for invoice details.</div>
    ...
    <section data-p="aging" id="panel-aging" role="tabpanel" aria-labelledby="tab-aging" hidden={tab !== "aging"}><div className="tw-note">Expand a client to see their invoices. Aging starts from each invoice’s due date; totals stay in the selected currency.</div><div className="tw-c"><Head title="A/R aging" subtitle={`Aged from due date · ${currency} · reference ${dateLabel(data.asOf)}`}><button className="tw-b sm" onClick={() => invoiceCsv(invoices)}>Export CSV</button></Head><AgingTree invoices={invoices} clients={clients} asOf={data.asOf} code={currency} /><div className="tw-note">Empty buckets show —. Current remains visible. Invoice and client balances use the same currency and reference date.</div></div></section>
    <section data-p="overdue" id="panel-overdue" role="tabpanel" aria-labelledby="tab-overdue" hidden={tab !== "overdue"}><div className="tw-note">Review overdue invoices, log contact with Remind, or use Record to enter money received. Logging contact does not send a message.</div><div className="tw-c"><Head title="Overdue management" subtitle={`${overdue.length} invoices · ${currency} ${money(summary.overdue)} past due`}><span className="tw-seg">{[["all", "All"], ["1", "1–30"], ["2", "31–60"], ["3", "61–90"], ["4", "90+"]].map(([key, label]) => <button key={key} aria-pressed={bucket === key} onClick={() => setBucket(key)}>{label}</button>)}</span><button className="tw-b sm" onClick={() => invoiceCsv(shownOverdue)}>Export CSV</button></Head>
      <Scroll width={1040}><Grid type="overdue" variant="tw-hr" cells={["Invoice", "Client", "Issued", "Due", "Days", "Bucket", "Outstanding", "Last contact", <Empty />]} />{shownOverdue.map(r => <Grid key={r.id} type="overdue" variant="tw-r wrn" cells={[<span className="tw-code">{documentLabel(r.document_number)}</span>, r.client_name, dateLabel(r.issue_date), dateLabel(r.due_date), daysOverdue(r.due_date, data.asOf), <span className="tw-p p-y">{BUCKETS[bucketIndex(r.due_date, data.asOf) ?? 0]} days</span>, <Value amount={r.outstanding} />, <span className={data.contacts[r.id] ? "tw-t" : "tw-miss"}>{dateLabel(data.contacts[r.id])}</span>, <span style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}><button className="tw-b sm" title="Log a reminder already sent" onClick={() => editFollowUp("contact", r.id, documentLabel(r.document_number) || r.id)}>Remind</button><button className="tw-b sm" onClick={() => record(r.id)}>Record</button><Link className="tw-b sm pri" href={`/billing/invoices/${r.id}`}>Open</Link></span>]} />)}<Grid type="overdue" variant="tw-ft" cells={[`${shownOverdue.length} invoices`, <Empty />, <Empty />, <Empty />, <Empty />, currency, <Value amount={shownOverdue.reduce((s, r) => s + r.outstanding, 0)} empty={!shownOverdue.length} />, <Empty />, <Empty />]} /></Scroll>
    ...
    <section data-p="stmt" id="panel-stmt" role="tabpanel" aria-labelledby="tab-stmt" hidden={tab !== "stmt"}><div className="tw-note">Search the client list and select a client to view their statement. With balance hides clients with no open balance; All shows everyone.</div><div className="tw-c"><Head title="Client statements" subtitle={`${clients.length} clients · ${clientList.withBalance} with an open balance`} /><div className="tw-stmt2"><div className="tw-stmt2__l"><div className="tw-stmt2__t"><input className="tw-in" aria-label="Find client" placeholder="Find client" value={statementSearch} onChange={e => setStatementSearch(e.target.value)} /><span className="tw-seg" style={{ marginTop: 7 }}><button aria-pressed={!statementAll} onClick={() => setStatementAll(false)}>With balance <em>{clientList.withBalance}</em></button><button aria-pressed={statementAll} onClick={() => setStatementAll(true)}>All <em>{clients.length}</em></button></span></div><div className="tw-stmt2__s">{clientList.visible.map(c => <button key={c.id} className={`tw-g tw-r${selectedClient === c.id ? " sel" : ""}`} style={{ "--cols": COLLECTION_COLS.client } as CSSProperties} aria-pressed={selectedClient === c.id} onClick={() => setStatementClient(c.id)}><span className="tw-t"><b>{c.name}</b><u>{c.rows.length ? `${c.rows.length} invoice${c.rows.length === 1 ? "" : "s"}` : "No invoice issued"}</u></span><span>{groupCurrency(c.rows).map(([code, rows]) => <span key={code} style={{ display: "block" }}><span className="tw-hint">{code}</span><Value amount={rows.reduce((sum, row) => sum + row.outstanding, 0)} /></span>)}{!c.rows.length && <Value amount={0} empty />}</span></button>)}{!clientList.visible.length && <div className="tw-note">No clients match. Clear the search or choose All.</div>}</div><div className="tw-stmt2__f">Sorted by outstanding in {currency}; other currencies stay separate. {!statementAll ? `${clients.length - clientList.withBalance} clients with no open balance are hidden. Switch to All to see them.` : "All clients are included."}</div></div><div className="tw-stmt2__r">
      ...
      {groupCurrency(statement).map(([code, rows]) => <div key={code}><div className="tw-note"><span className="tw-cc">{code}</span></div><Scroll width={920}><Grid type="statement" variant="tw-hr" cells={["Invoice", "Issued", "Due", "Campaign", "Days", "Invoiced", "Open"]} />{rows.map(r => <Grid key={r.id} type="statement" variant="tw-r" cells={[<Link className="tw-code" href={`/billing/invoices/${r.id}`}>{documentLabel(r.document_number)}</Link>, dateLabel(r.issue_date), dateLabel(r.due_date), r.campaign_name, <span className="tw-v">{daysOverdue(r.due_date, data.asOf) ?? "not set"}</span>, <Value amount={r.total} />, <Value amount={r.outstanding} />]} />)}<Grid type="statement" variant="tw-ft" cells={[`${rows.length} invoices`, <Empty />, <Empty />, code, <Empty />, <Value amount={rows.reduce((s, r) => s + r.total, 0)} />, <Value amount={rows.reduce((s, r) => s + r.outstanding, 0)} />]} /></Scroll></div>)}{!statement.length && <div className="tw-note">No invoice has been issued for this client. A statement will be available after the first invoice is issued.</div>}
    </div></div></div></section>
    <section data-p="fcast" id="panel-fcast" role="tabpanel" aria-labelledby="tab-fcast" hidden={tab !== "fcast"}><div className="tw-note">Forecasts use scheduled invoice due dates. These amounts are expected receipts, not recorded payments.</div><div className="tw-c"><Head title="Collections & cashflow forecast" subtitle={`From open ${currency} A/R due dates · no prediction`} />{(() => {
