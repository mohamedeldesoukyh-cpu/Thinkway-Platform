...
export const COLLECTION_COLS = {
  ...
  overdue: "88px minmax(148px,1.2fr) 96px 96px 78px 106px 128px 112px 166px",
  statement: "96px 106px 106px minmax(140px,1fr) 126px 126px",
...
function Value({ amount, empty = false }: { amount: number; empty?: boolean }) { return <span className={`tw-v${empty || amount === 0 ? " z" : ""}`} style={{ display: "block", textAlign: "right" }}>{empty ? "—" : money(amount)}</span>; }
...
function ReceiptForm({ invoices, preferred, asOf }: { invoices: CollectionInvoiceRow[]; preferred: string; asOf: string }) {
  ...
  useEffect(() => { if (preferred) setId(preferred); }, [preferred]);
...
export function CollectionsRedesign({ data }: { data: CollectionsPageData }) {
  ...
  const payRows = payables.filter(r => payStatus === "all" || (payStatus === "paid" ? r.status === "paid" : r.status !== "paid")).filter(r => `${r.influencer_name} ${r.campaign_name} ${r.currency}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => a.currency.localeCompare(b.currency) || b.agreed_fee - a.agreed_fee);
  const pageCount = Math.max(1, Math.ceil(payRows.length / 10));
  const currentPage = Math.min(page, pageCount);
  const selectedClient = statementClient || clients[0]?.id || "";
  ...
  return <div className="collections-suite" style={{ minWidth: 0, maxWidth: "100%", fontFamily: "var(--font-sans, Geist), sans-serif" }}>
    <div className="tw-mast">
      ...
      <div className="tw-mr" role="tablist" aria-label="Collections sections">{COLLECTION_TABS.map(([key, label]) => <button key={key} id={`tab-${key}`} role="tab" aria-controls={`panel-${key}`} aria-selected={tab === key} className={`tw-b sm${tab === key ? " pri" : ""}`} data-s={key} onClick={() => navigate(key)}>{label}{(key === "aging" || key === "overdue" || key === "pay") && <span className="tw-cc">{key === "aging" ? open.length : key === "overdue" ? overdue.length : payables.filter(r => r.status !== "paid").length}</span>}</button>)}<span className="tw-sp" />
        <select className="tw-in" aria-label="Client" style={{ width: 150, height: 27 }} value={client} onChange={e => { setStatementClient(e.target.value); setPage(1); navigate(tab, e.target.value); }}><option value="">All clients</option>{data.clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
        ...
        <button className="tw-b sm" onClick={() => { setBucket("all"); setPayStatus("open"); setSearch(""); setPage(1); navigate(tab, ""); }}>Reset</button>
      </div>
    ...
    <section data-p="pay" id="panel-pay" role="tabpanel" aria-labelledby="tab-pay" hidden={tab !== "pay"}>
      <div className="tw-c"><Head title="Vendor payables" subtitle={`${payables.length} lines · ${payables.filter(r => r.status !== "paid").length} open · ${campaigns.length} campaigns`}><span className="tw-seg">{["open", "paid", "all"].map(s => <button key={s} aria-pressed={payStatus === s} onClick={() => { setPayStatus(s); setPage(1); }}>{s[0].toUpperCase() + s.slice(1)}</button>)}</span></Head><div className="tw-note">Subtotalled per currency. No combined figure is shown and no exchange-rate conversion is applied.</div><div className="tw-ms2">{totals.flatMap(t => [[`Open · ${t.currency}`, t.pending, t.pendingCount], [`Paid · ${t.currency}`, t.paid, t.paidCount]] as const).map(([label, value, count]) => <div key={label}><i>{label}</i><b className={value === 0 ? "s" : ""}>{count ? money(value) : "—"}</b></div>)}</div></div>
      <div className="tw-c"><Head title="By campaign" subtitle="A separate row for each campaign and currency" /><Scroll width={740}><Grid type="campaign" variant="tw-hr" cells={["Campaign", "Lines", "Ccy", "Open", "Paid"]} />{campaigns.flatMap(c => groupCurrency(payables.filter(r => r.campaign_name === c)).map(([code, rows]) => { const t = payableCurrencyTotals(rows)[0]; return <Grid key={`${c}-${code}`} type="campaign" variant="tw-r" cells={[c, rows.length, <span className="tw-cc">{code}</span>, <Value amount={t.pending} empty={!t.pendingCount} />, <Value amount={t.paid} empty={!t.paidCount} />]} />; }))}</Scroll></div>
      <div className="tw-c"><Head title="Payable balances" subtitle={`Showing ${payRows.length ? (currentPage - 1) * 10 + 1 : 0}–${Math.min(currentPage * 10, payRows.length)} of ${payRows.length} · sorted within currency`}><input className="tw-in" aria-label="Find vendor" placeholder="Find vendor" style={{ width: 170, height: 27 }} value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} /><button className="tw-b sm" onClick={() => csv("vendor-payables.csv", [["Vendor", "Campaign", "Status", "Currency", "Fee", "Due"], ...payRows.map(r => [r.influencer_name, r.campaign_name, r.status, r.currency, r.agreed_fee, data.dueDates[r.id] ?? ""])])}>Export CSV</button></Head><Scroll width={900}><Grid type="payable" variant="tw-hr" cells={["Vendor", "Campaign · due", "Status", "Ccy", "Fee", <Empty />]} />{payRows.slice((currentPage - 1) * 10, currentPage * 10).map(r => <Grid key={r.id} type="payable" variant={`tw-r${r.status === "paid" ? "" : " wrn"}`} cells={[r.influencer_name, <span className="tw-t">{r.campaign_name}<br /><span className={data.dueDates[r.id] ? "tw-hint" : "tw-miss"}>Due: {dateLabel(data.dueDates[r.id])}</span></span>, <span className={`tw-p ${r.status === "paid" ? "p-g" : "p-y"}`}>{r.status}</span>, <span className="tw-cc">{r.currency}</span>, <Value amount={r.agreed_fee} />, <span style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}><button className="tw-b sm" onClick={() => editFollowUp("due", r.id, r.influencer_name)}>Schedule</button>{data.vendorIos[r.id] ? <Link className="tw-b sm" href={`/ios/vendor/${data.vendorIos[r.id]}/preview`}>Open IO</Link> : <span className="tw-miss">IO not set</span>}</span>]} />)}</Scroll><div className="tw-ch"><button className="tw-b sm" disabled={currentPage === 1} onClick={() => setPage(p => p - 1)}>Previous</button><span>Page {currentPage} of {pageCount}</span><button className="tw-b sm" disabled={currentPage >= pageCount} onClick={() => setPage(p => p + 1)}>Next</button></div><div className="tw-note">{payables.filter(r => !data.dueDates[r.id]).length} payable lines have no scheduled due date. Use Schedule to record one; scheduling does not initiate a payout.</div></div>
    </section>
