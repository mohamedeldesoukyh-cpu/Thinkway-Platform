        ...
        <tbody>{visible.map(row => {
          ...
          return <Fragment key={row.assignmentId}>
            <tr className={`${missing ? 'bank-warning' : ''} ${selected.has(row.assignmentId) ? 'selected' : ''}`}>
              ...
              <td className="cp-code">{row.ioNumber}<small>Creator Inv# {row.creatorInvoice?.number||'Not added'}</small></td>
            ...
            {(open || errors.length > 0) && <tr className="cp-detail-row" id={`payment-details-${row.assignmentId}`}><td colSpan={11}>{open && <div className="cp-balances"><span>Paid <b>{amount(row.paid,row.currency)}</b></span><span>Pending exports <b>{amount(row.reserved,row.currency)}</b></span><span>Remaining after payment <b>{amount(calculation.remaining * calculation.rate,draft.currency)}</b>{draft.currency !== row.currency && <small>{amount(calculation.remaining,row.currency)} original</small>}</span></div>}{errors.map(error => <p key={error} role="alert" className="cp-error">{error}</p>)}</td></tr>}
