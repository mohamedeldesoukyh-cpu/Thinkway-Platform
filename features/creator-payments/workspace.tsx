...
import { loadCreatorPayments, saveCreatorPaymentPlans, exportCreatorPayments, downloadCreatorPaymentBatch, confirmCreatorPayment } from './actions';
...
export function CreatorPaymentsWorkspace({ campaignId, creatorId }: {
    ...
    const dirtyRows = changedPaymentPlans(rows, drafts);
    async function savePlans() {
        if (!dirtyRows.length || busy) return;
        const saved = dirtyRows.map(row => ({ assignmentId: row.assignmentId, draft: drafts[row.assignmentId] }));
        setBusy(true);
        loadVersion.current++;
        try {
            const result = await saveCreatorPaymentPlans(saved);
            if (!result.ok) { toast.error(result.message); return; }
            loadVersion.current++;
            setRows(current => current.map(row => {
                const item = saved.find(item => item.assignmentId === row.assignmentId);
                return item ? { ...row, savedDraft: item.draft } : row;
            }));
            setDrafts(current => Object.fromEntries(Object.entries(current).filter(([id, draft]) => !saved.some(item => item.assignmentId === id && item.draft === draft))));
            toast.success('Payment plans saved. Confirm bank results to mark payments as paid.');
            await reload(true);
        } catch { toast.error('Could not save payment plans. Your edits are still here.'); }
    ...
    async function exportBatch() { if (dirtyRows.length) { toast.error('Save payment changes before exporting.'); return; } if (!chosen.length || !oneCampaign)
    ...
    {!!dirtyRows.length && <div className="cp-save-bar" role="status"><span><b>{dirtyRows.length} unsaved payment changes</b> · Amounts are a preview until you save.</span><div><Button variant="outline" disabled={busy} onClick={() => setDrafts({})}>Discard</Button><Button disabled={busy || !canWrite} onClick={() => void savePlans()}>{busy ? 'Saving…' : 'Save changes'}</Button></div></div>}
