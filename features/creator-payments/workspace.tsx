...
export function CreatorPaymentsWorkspace({ campaignId, creatorId }: {
    ...
    <Dialog open={!!bankRow} onOpenChange={open => { if (!open)
        setBankRow(null); }}><DialogContent className="creator-bank-dialog"><DialogTitle className="sr-only">Creator bank details</DialogTitle>{bankRow && <AaibBankEditor key={bankRow.creatorId} creatorId={bankRow.creatorId} row={bankRow} initial={bankRow.bank} onSaved={() => { setBankRow(null); void reload(); }}/>}</DialogContent></Dialog>
