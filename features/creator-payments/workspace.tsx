...
export function CreatorPaymentsWorkspace({ campaignId, creatorId }: {
    ...
    {!!dirtyRows.length && !chosen.length && <div className="cp-save-bar cp-save-bottom"><span>Record payments already made, with their payment dates.</span><div><Button variant="outline" disabled={busy} onClick={() => { paymentRequest.current=null; setDrafts({}); }}>Discard</Button><Button disabled={busy || !canWrite} onClick={() => void savePlans()}>{busy ? 'Saving…' : 'Save payments'}</Button></div></div>}
