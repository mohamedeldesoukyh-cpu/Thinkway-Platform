...
export function InvoiceVatEditor({row,canWrite,onSaved}:{row:PaymentRow;canWrite:boolean;onSaved:(row?:PaymentRow)=>void|Promise<void>}){
 ...
 {row.vat!==(vat?14:0)&&<label className="mb-3 block text-sm">Finance correction reason<textarea name="overrideReason" required minLength={5} maxLength={1000} className="mt-1 block w-full rounded border p-2" placeholder="Explain why creator VAT needs correcting"/><span className="block text-xs">Locked assignments require Finance override permission. This correction and its reason are recorded in the audit history.</span></label>}
