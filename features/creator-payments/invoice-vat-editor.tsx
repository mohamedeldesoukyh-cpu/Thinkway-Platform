...
export function InvoiceVatEditor({row,canWrite,onSaved}:{row:PaymentRow;canWrite:boolean;onSaved:(row?:PaymentRow)=>void|Promise<void>}){
 ...
 <form onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);start(async()=>{const r=await saveCreatorPaymentInvoice({assignmentId:row.assignmentId,number:String(f.get('number')||''),date:String(f.get('date')||''),country:String(f.get('country')||''),vat,revision:row.creatorInvoice?.revision??0,expectedVat:row.vat,overrideReason:String(f.get('overrideReason')||'')});if(!r.ok){setError(r.message);return;}toast.success('Invoice and VAT saved. Payment balance updated.');setOpen(false);await onSaved(row);});}}>
 ...
 {<label className="mb-3 block text-sm">Finance correction reason<textarea name="overrideReason" required={row.vat!==(vat?14:0)} minLength={5} maxLength={1000} className="mt-1 block w-full rounded border p-2" placeholder="Explain why creator VAT needs correcting"/><span className="block text-xs">Locked assignments require Finance override permission. This correction and its reason are recorded in the audit history.</span></label>}
