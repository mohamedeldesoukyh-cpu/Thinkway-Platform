...
- **Quotation avatars:** Prior fix still left CDN-only creators blank on Prod — workspace stabilize used the user session client, but `creator-avatars` uploads are **service_role-only**, and Instagram OG scrape is blocked. Fix: admin client + Apify fallback (`repairCreatorAvatarDurably`), `POST /api/admin/creators/repair-avatars`, script `scripts/repair-prod-quotation-avatars.mjs` (needs `PROD_SUPABASE_SERVICE_ROLE_KEY`). QT-2026-0009-V2 failing handles still CDN-only until Prod data repair runs.
