...
- **Commercial SSOT Phases 1–4 done (feature-complete):** Bidirectional Master sync by CML ID; dirty-field writes; field-level audit; registry-driven Derived recalc; platform Finance Lock `Campaign.isFinanceLocked`; Commercial Revision workflow (create/submit/approve-apply/reject, version history, UI from finance-lock gate). Migration `20260729210000_commercial_revisions.sql`. Tests: `npm run test:commercial-ssot-phase4`. Spec: `docs/architecture/COMMERCIAL_SSOT_QUOTE_CAMPAIGN.md` (D-COMM). Next: E2E/UAT — no further Commercial SSOT feature expansion unless requested.
